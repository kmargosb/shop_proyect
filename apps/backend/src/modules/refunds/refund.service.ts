...
export const RefundService = {
  async createRefund(
    ...
    if (!order.stripePaymentIntentId) throw new Error("Order has no payment intent");
    ...
    for (const item of items) {
      ...
      if (remainingQuantity <= 0) throw new Error("Item already fully refunded");
    ...
    await prisma.orderEvent.create({
      data: { orderId, type: "REFUND_CREATED", message: "Refund created" },
    });

    getIO().emit("orderUpdated", { orderId });

    return { refundId: dbRefund.id, amount: dbRefund.amount, status: dbRefund.status };
  },
  ...
  async approveRefund(refundId: string) {
    ...
    if (refund.status !== "PENDING_REVIEW") throw new Error("Refund already processed");
    if (!refund.order.stripePaymentIntentId) throw new Error("Order has no payment intent");
    ...
    const updatedRefund = await prisma.refund.update({
      where: { id: refundId },
      data: { status: "SUCCEEDED", stripeRefundId: stripeRefund.id, reviewedAt: new Date() },
    });
    ...
    for (const item of refund.items) {
      ...
      try {
        await InventoryCache.incrementStock(productId, item.quantity);
      } catch (error) {
        console.error("Redis stock restore error:", error);
      }
    }

    const approvedAggregate = await prisma.refund.aggregate({
      where: { orderId: refund.orderId, status: "SUCCEEDED" },
      _sum: { amount: true },
    });

    const totalRefunded = approvedAggregate._sum.amount ?? 0;

    await prisma.order.update({
      where: { id: refund.orderId },
      data: {
        status:
          totalRefunded >= refund.order.totalAmount ? "REFUNDED" : "PARTIALLY_REFUNDED",
      },
    });
    ...
    getIO().emit("orderUpdated", { orderId: refund.orderId });
