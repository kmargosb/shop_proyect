...
export default function AdminProducts() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-400 sm:text-base">
              Gestiona inventario, precios, visibilidad y acciones rápidas con una experiencia
              preparada para catálogos grandes.
            </p>
          </div>
          <button
      ...
      {creating && (
        <CreateProductModal onClose={() => setCreating(false)} onCreated={loadProducts} />
      )}
      {productToDelete && (
        <ConfirmDeleteModal
...
function getProductStock(product: Product) {
  return (
    product.variants?.reduce(
      (total, variant) =>
        total + (getSafeNumber(variant.stock) - getSafeNumber(variant.reservedStock)),
      0,
    ) ?? 0
  );
}
  ...
  return (
    <article className="rounded-3xl border border-white/10 bg-neutral-950/80 p-4 shadow-xl shadow-black/20">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
        ...
        <Metric label="Stock" value={getProductStock(product)} />
...
function ProductStat({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-neutral-950/80 p-3 shadow-xl shadow-black/20 sm:rounded-3xl sm:p-5">
      ...
      <p className={`mt-1 text-lg font-semibold sm:mt-2 sm:text-2xl ${tones[tone]}`}>{value}</p>
      <p className="mt-1 line-clamp-1 text-[11px] text-neutral-500 sm:text-xs">{helper}</p>
    </div>
  );
}
  ...
  return (
    ...
    >
      {options.map(([optionValue, optionLabel]) => (
        <option key={optionValue} value={optionValue} className="bg-neutral-950">
          {optionLabel}
        </option>
      ))}
    </select>
...
function EmptyProducts({ onCreate }: { onCreate: () => void }) {
  return (
    <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.02] p-10 text-center">
      ...
      <p className="mt-4 font-semibold text-neutral-200">No hay productos para estos filtros</p>
      <p className="mt-2 text-sm text-neutral-500">Ajusta la búsqueda o crea un producto nuevo.</p>
      <button
        onClick={onCreate}
